...
export class DataSourceController {
    ...
    static async GetDataSourcesNamesForDashboard(req: Request, res: Response, next: NextFunction) {
        ...
        try {
            //si no lleva filtro, pasamos directamente a recuperarlos todos           
            /*EDA   const datasources = JSON.stringify(filter) !== '{}' ? */
            /*EDA   await DataSource.find({ $or: Object.entries(filter).map(([clave, valor]) => ({ [clave]: valor })) }, '_id ds.metadata.model_name ds.metadata.model_granted_roles ds.metadata.model_owner ds.metadata.ia_visibility', options).exec() : */
            /*EDA  await DataSource.find({}, '_id ds.metadata.model_name ds.metadata.model_granted_roles ds.metadata.model_owner ds.metadata.model_description ds.metadata.ia_visibility', options).exec(); */

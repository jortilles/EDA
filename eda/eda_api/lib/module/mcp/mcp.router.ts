...
McpRouter.post('/', async (req: Request, res: Response) => {
    ...
    // Intentar recuperar el usuario del header x-user-token o Authorization: Bearer <token>
    let requestUser: any = null;
    const rawUserToken   = req.headers['x-user-token'] as string;
    const rawBearer      = (req.headers['authorization'] as string)?.replace(/^Bearer\s+/i, '');
    const userToken      = rawUserToken || rawBearer || '';
    const tokenSource    = rawUserToken ? 'x-user-token' : rawBearer ? 'Authorization Bearer' : 'ninguno';
    console.log('[MCP] token presente:', !!userToken, '| fuente:', tokenSource);
    if (userToken) {
        console.log('[MCP] Intentando verificar token...');
        try {
            const decoded: any = jwt.verify(userToken, SEED);
            requestUser = decoded.user;
            console.log('[MCP] token OK — usuario:', requestUser?.email ?? '(sin email)', '| id:', requestUser?._id ?? '(sin id)', '| role:', requestUser?.role ?? '(sin role)');
        } catch (e: any) {
            console.warn('[MCP] token inválido:', e.message, '→ fallback a loginInternal');
        }
    } else {
        console.log('[MCP] Sin token → se usará loginInternal con el usuario del config');
    }
...
McpRouter.post('/chat', authGuard, async (req: Request, res: Response) => {
    ...
    try {
        ...
        console.log('[CHAT] usuario:', reqUser?.email ?? reqUser?._id ?? '(ninguno)', '| token generado:', !!userToken);
        ...
        const systemPrompts = [buildEnhancedSystemPrompt(reqUser || {}), CHAT_MAIN_SYSTEM_PROMPT];

        // ── Rama nativa MCP (Anthropic) ────────────────────────────────────────
        if (provider.turnWithNativeMCP) {
            console.log('[CHAT] Usando MCP nativo Anthropic | MCP_URL:', MCP_URL);
            const result = await provider.turnWithNativeMCP(
                systemPrompts,
                history,
                MCP_URL,
                userToken,
            ...
            const { text, options } = MCPUtils.extractOptionsFromResponse(result.text);
            console.log('[CHAT] MCP nativo finalizado | chars:', text.length, '| opciones:', options?.length ?? 0);
            MCPUtils.finalizeChatContext(ctx);
            const payload: any = { ok: true, response: text };
            if (options && options.length > 1) payload.options = options;
            return res.status(200).json(payload);
        }

        // ── Rama manual (OpenAI / Bedrock) ─────────────────────────────────────
        console.log('[CHAT] Usando loop manual | provider:', aiConfig.PROVIDER);
        ...
        try {
            while (iterations < MAX_ITERATIONS) {
                ...
                const turnResult = await provider.turn(systemPrompts, history, normalizedTools, MAX_TOKENS || 4096);

...
export class JSONWebServiceConnection extends AbstractConnection {
	...
	async generateDataModel(optimize: number, filter: string, name: string): Promise<any> {
	...
	async execQuery(query: any): Promise<any> {
		try {
			...
			const permissions = this.getUserPermissions(modelPermissions, this.user, this.groups);
			...
			if (aggregationInfo.aggFields.length > 0) {
				response = this.aggregate(response, aggregationInfo.nonAggFields, aggregationInfo.aggFields, fields);
			}
			...
			if (response.length == 0) {
				...
				fields.forEach((field: any) => {
					if (field.column_type == 'numeric' &&
						['count', 'count_distinct'].includes(field.aggregation_type)){
						doit = 1
				...
				fields.forEach((field: any) => {
					if (field.column_type == 'numeric' && field.whatif_column) {
						response.forEach((res: any) => {
							if (res[field.column_name]) {
								res[field.display_name.trim()] = this.execWhatIfExpression(field.whatif?.operator, field.whatif?.value, res[field.column_name]);
							}
						});

						field.column_name = field.display_name.trim();
					}
 				})
			}
	...
	private execWhatIfExpression(whatIfOperator: string, whatIfValue: string, value: string) {
		let res = 0;
		switch (whatIfOperator) {
			case "+":
				res = Number(value) + Number(whatIfValue);
			  break;
			case "-":
				res = Number(value) - Number(whatIfValue);
			  break;
			...
			case "/":
			  ...
			  break;
			default:
			  console.log("whatIfOperator invalid");
		  }
	...
	private filter = (data: any[], filters: any[]) => {
	...
	private getAggregationInfo = (fields: any[]) => {
	...
	private formatDates = (arr: any[], fieldsToFormat: any[]) => {
		arr.forEach(row => {
	...
	private aggregate(arr: any[], groupByCols: any[], aggregateCols: any[], fields) {
		const subSet = (o: Object, keys: any[]) =>
	...
	private aggregateValue = (value: any, total: number, aggType: string, countDistinctValues: any) => {
	...
	private getSortInfo = (fields: any[]) => {
		return fields.filter(field => field.ordenation_type !== 'No')
	}

	private sort = (data: any[], sortFields: any[]) => {
		let evalChain = []

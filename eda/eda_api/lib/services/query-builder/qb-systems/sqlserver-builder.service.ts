      ...
      if (filter_elements.length === 0) {
        ...
        if (filter_elements[0].value1.length === 1) {
          ...
          if (filter_column_type === 'date') {
            filter_elements_value = filter_type === 'between'
              ? ` CAST('${filter_elements[0].value1[0]}' AS date) and CAST('${filter_elements[1]?.value2?.[0]} 23:59:59' AS datetime)`
              : (filter_type === 'in' || filter_type === 'not_in') ? `(CAST('${filter_elements[0].value1[0]}' AS date))` : `CAST('${filter_elements[0].value1[0]}' AS date)`;
          }
        } else {
          ...
          if (filter_column_type === 'text' || filter_column_type === undefined) {
            ...
            vals.forEach((e: any, i: number) => { filter_elements_value += `CAST('${e}' AS date)${i === vals.length - 1 ? ')' : ','}`; });
          }
        }
      }

...
export class PgBuilderService extends QueryBuilderService {
 ...
 public normalQuery(columns: string[], origin: string, dest: any[], joinTree: any[], grouping: any[], filters: any[], havingFilters: any[],
  ...
  public getSortedFilters(sortedFilters: any[], filters: any[]): any {
    ...
    // Push permissions as regular filters
    if (this.permissions.length > 0) {
      this.permissions.forEach(permission => { filters.push(permission); });
    }

    // Adding valueListSource to sortedFilters items
    ...
    // Remove empty global filters and adjust positions
    ...
    // Handle null_or_empty and is_null special cases

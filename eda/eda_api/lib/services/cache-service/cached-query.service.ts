...
export class CachedQueryService {
  ...
  static async getQuery(queryHash) {
    try {
      const query = await CachedQuery.findOne({ 'cachedQuery.hashedQuery': queryHash }).exec();
      return query;
  ...
  static async updateQueries() {
    ...
    try {
      ...
      const cachedModels = models.filter(model => model.ds.metadata.cache_config)
        .map(model => { return { model_id: `${model._id}`, cache_config: model.ds.metadata.cache_config } });

      const queries = await CachedQuery.find({ 'cachedQuery.model_id': { $in: cachedModels.map(model => model.model_id) } });

      let updatedQueries = 0;

      queries.forEach(async query => {

        const cache_config = cachedModels.filter(m => m.model_id = query.cachedQuery.model_id).map(_ => _.cache_config)[0];
        let shouldUpdate = false;

        if (cache_config.units === 'hours') {

          shouldUpdate = SchedulerFunctions.checkScheduleHours(cache_config.quantity, query.cachedQuery.lastUpdated);

        } else if (cache_config.units === 'days') {

          shouldUpdate = SchedulerFunctions.checkScheduleDays(cache_config.quantity, cache_config.hours, cache_config.minutes, query.cachedQuery.lastUpdated);
  ...
  static hoursToInt(date: Date) {
    ...
    let hour = date.getHours(), hourStr;
    hourStr = hour > 10 ? `0${hour}` : `${hour}`;
    let minutes = date.getMinutes(), minutesStr;

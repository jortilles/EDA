...
import DataSourceSchema from '../../../module/datasource/model/datasource.model';
...
export class CleanModel {
    ...
    public async cleanModel(main_model : any) : Promise<any> {
        ...
        for (let i=0;i<roles.length;i++) {
            ...
            if (i==0) {
                ...
                if( _.isEmpty(match) == false ){
                    if(roles[i].value && match.value ){
                        roles[i].value.forEach((e: any, i: number)=> {
                            if( e != match.value[i]){
                                match =  false;
                            }
                        });
            ...
            const mgsmap = _.cloneDeep(finder.map(e => e.ds.metadata.model_granted_roles));
            ...
            if(mgsmap.length!==0) {
                const userRoles = mgsmap[0].filter( (r:any) => {
                    return r?.source === 'SDA' && !r.groupsName.find( (e: string) => e.startsWith('SCRM_'))
                });

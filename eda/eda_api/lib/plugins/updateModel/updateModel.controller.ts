...
import User, { IUser } from "../../module/admin/users/model/user.model";
...
export class updateModel {
  ...
  static async update(req: Request, res: Response) {
    ...
    insertServerLog(req, 'info', 'UpdateModelStarted', getUpdateModelActor(req), buildUpdateModelLogType('start', 'connection_opened'));
    ...
    try {
                                        ...
                                        await connection.query(query).then(async customUserPermissionsValue => {
                                          ...
                                          .then(async cacheConfigSDA => {
                                            ...
                                            try {
                                              ...
                                              insertServerLog(req, 'info', 'UpdateModelUsersAndGroupsSynced', getUpdateModelActor(req), buildUpdateModelLogType('users_groups', `users:${(users_crm || []).length}--roles:${(roles || []).length}`));
                                            } catch (e) {
                                              console.log("Error 1", e);
                                              insertServerLog(req, 'error', 'UpdateModelUsersAndGroupsSyncFailed', getUpdateModelActor(req), buildUpdateModelLogType('users_groups', stringifyLogError(e)));
                                            ...
                                            try {
                                              ...
                                              insertServerLog(req, 'info', 'UpdateModelRolesMapped', getUpdateModelActor(req), buildUpdateModelLogType('roles', `granted_roles:${(grantedRolesAt || []).length}`));
                                            } catch (e) {
                                              console.log("Error 2", e);
                                              insertServerLog(req, 'error', 'UpdateModelRolesMappingFailed', getUpdateModelActor(req), buildUpdateModelLogType('roles', stringifyLogError(e)));
                                              res.status(500).json({ status: "ko" });
                                            }
                                            ...
                                            try {
                                              ...
                                              insertServerLog(req, 'info', 'UpdateModelDataModelBuilt', getUpdateModelActor(req), buildUpdateModelLogType('build_model', `tables:${(tables || []).length}--columns:${(columns || []).length}--relations:${(relations || []).length}`));
                                            } catch (e) {
                                              console.log("Error 3", e);
                                              insertServerLog(req, 'error', 'UpdateModelDataModelBuildFailed', getUpdateModelActor(req), buildUpdateModelLogType('build_model', stringifyLogError(e)));
                                              res.status(500).json({ status: "ko" });
                                            }
                                          });
                                        });
      ...
      console.log("Error : ", e);
      insertServerLog(req, 'error', 'UpdateModelFailed', getUpdateModelActor(req), buildUpdateModelLogType('unhandled', stringifyLogError(e)));
    }
  }
  ...
  */
  ...
  static createModel(
    ...
    this.extractJsonModelAndPushToMongo(req, destTables, grantedRoles, res, cache_configSDA);
  ...
  static async extractJsonModelAndPushToMongo(req: Request, tables: any, grantedRoles: any, res: any, cache_configSDA: any) {
    ...
    try{
      ...
      let sda_config_cache_enabled_value = cache_configSDA.find( (v: any) => v.key === 'sda_config_cache_enabled').value
    ...
    try {
      ...
      insertServerLog(req, 'info', 'UpdateModelCompleted', getUpdateModelActor(req), buildUpdateModelLogType('completed', `tables:${(tables || []).length}--granted_roles:${(grantedRoles || []).length}`));
      res.status(200).json({ status: "ok" });
    } catch (e) {
      console.log("Error :", e);
      insertServerLog(req, 'error', 'UpdateModelPushFailed', getUpdateModelActor(req), buildUpdateModelLogType('push_model', stringifyLogError(e)));
      res.status(500).json({ status: "ko" });
    }
  }
}

function sanitizeLogSegment(value: any) {
  return (value || '-').toString().replace(/\|,\|/g, ' ').replace(/\r|\n/g, ' ').replace(/--/g, '-');
}

function buildUpdateModelLogType(stage: string, detail?: string) {
  const safeStage = sanitizeLogSegment(stage);
  if (!detail) return `${safeStage}`;
  const safeDetail = sanitizeLogSegment(detail);
  return `${safeStage}--${safeDetail}`;
}

function stringifyLogError(error: any) {
  if (!error) return 'unknown_error';
  if (error.message) return sanitizeLogSegment(error.message);
  return sanitizeLogSegment(error.toString());
}

function getUpdateModelActor(req: Request) {
  const requestUser = (req as any) && (req as any).user && (req as any).user.name ? (req as any).user.name : undefined;

...
export class ChartUtilsService {
    ...
    public chartTypes: EdaChartType[] = [
        ...
        { label: $localize`:@@chartTypesPyramid:Gráfico de Piramide`, value: 'bar', subValue: 'pyramid', icon: 'pi pi-exclamation-triangle', ngIf: true, tooManyData: true },
    ...
    */
    public transformDataQuery(type: string, subType: string,  values: any[], dataTypes: string[], dataDescription: any, isBarline: boolean, numberOfColumns: number) {
        ...
        if (type === 'doughnut' || type === 'polarArea') {
            ...
            // typeof check matters here: isNaN(false) is false (Number(false) coerces to 0), so a
            // stray boolean sentinel for "not set" would otherwise pass this check and silently
            // zero out num_cols.
           ...
           if( this.esEntero(distinctNumbers)){
            ...
            _output[1] = [{
                data: new_data,
                // This branch is only ever reached for histogram queries (exactly 1 numeric
                // column) - renderBar() in panel-chart.component.ts already rewrites this name
                // into a full description ("Número de X en este rango") before calling
                // transformDataQuery, so prefixing histoGramRangesTxt ("Rango") here on top of
                // that produced a doubled, confusing label ("Rango - Número de X en este rango").
    ...
    private  generateGruposOneForHistogram(num_cols,min ):any[] {
        // Must produce exactly num_cols labels, one per bin generated by
        // generateNewDataOneForHistogram (mi_salto = min + i for i in [0, num_cols)) - the old
        // `for(i=min; i<=num_cols; i++)` only happened to line up when min===1; for any other
        // min (0, negative, >1...) it produced the wrong count/range and left labels/data
        // mismatched, causing bars to render against the wrong bucket or not at all.
        let grupos = [];
        for(let i=0; i<num_cols; i++){
            grupos.push(min+i);
        }
